...
void node_dump3(node_t *nd) {
    if(!nd) return;
    int i;
    if(nd) {
        LOG_INFO("{\n\"name\": \"%s\",\n\"optype\": \"%s\",\n\"nparam\":%d,\n", nd->name, op_name(nd->op->type) ? op_name(nd->op->type) : "Uninit" , node_get_nparam(nd));
        LOG_INFO("\"in\": [");
        for(i = 0; i < nd->nin; i++) {
            LOG_INFO("%d", nd->in[i]->index);
            if(i != nd->nin - 1) {
                LOG_INFO(",");
            }
        }
        ...
        if(vector_size(nd->attr_vec) > 0) {
            for(i = 0; i < vector_size(nd->attr_vec); i++) {
                attribute_t* attr = nd->attr_vec[i];
                attribute_dump3(attr);
                if(i != vector_size(nd->attr_vec) - 1){
                    LOG_INFO(",\n");
                }
            }
        }
