...
node_t* graph_add_conv2d(graph_t *g, int out_channels, int64_t kernel_shape[2], int64_t strides[2], int64_t pads[4], int64_t dilations[2], int group, char* auto_pad, const char* activation) {
    ...
    if(dilations) {
        dilations_attr = attribute_ints("dilations", dilations, 2);
    }
    if(auto_pad) {
        auto_pad_attr = attribute_string("auto_pad", auto_pad, strlen(auto_pad));
    }
    node_t* nd = graph_add_layer(g, OP_TYPE_CONV, (tensor_t*[]){last, kernel}, 2, 1,
        (attribute_t*[]){group_attr, kernel_shape_attr, strides_attr, pads_attr, dilations_attr, auto_pad_attr}, 6);
    // Activation
    graph_add_activation(g, activation);
    return nd;
}
...
void graph_dump1(graph_t* g) {
    ...
    char* mem = sys_mem_size(sum_nmemory);
    LOG_INFO("| --------------------------------------------------------------------------- |\n");
    LOG_INFO("| + Param : %d \n", sum_nparam);
    LOG_INFO("| + Mem   : %s \n", mem);
    LOG_INFO("| --------------------------------------------------------------------------- |\n");
...
void graph_dump3(graph_t* g) {
    if(!g) return;
    LOG_INFO("{\n\"name\": \"%s\",\n", g->name);
    LOG_INFO("\"nnode\": %u,\n\"nodes\": [\n", g->nnode);
    for(int i = 0; i < g->nnode; i++) {
        node_dump3(g->nodes[i]);
        if(i != g->nnode - 1){
            LOG_INFO(",\n");
        }
    }
    LOG_INFO("],\n");
    uint16_t ntensor_param = 0;
    LOG_INFO("\"ntensor\": %u,\n\"tensors\": [\n", g->ntensor);
    for(int i = 0; i < g->ntensor; i++) {
        tensor_dump3(g->tensors[i]);
        if(i != g->ntensor - 1){
            LOG_INFO(",\n");
        }
        if(g->tensors[i]->is_param) {
            ntensor_param++;
        }
    }
    LOG_INFO("],\n\"ntensor_param\": %u\n", ntensor_param);
...
void graph_export_dot(graph_t *g, char *path) {
    ...
    char* dot_buf = malloc(1024 * 1024 * sizeof(char));
    dot_buf[0] = '\0';
    dot_graph(dot_buf, g);
    fprintf(f, "%s", dot_buf);
    free(dot_buf);
