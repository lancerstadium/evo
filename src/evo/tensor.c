...
static inline void tensor_init(tensor_t *ts, int idx, int type) {
    ...
    ts->is_scalar = 0;      // ts is not scaler
    // dim
...
tensor_t* tensor_hist(tensor_t* ts, float qmin, float qmax, int bin) {
    if(!ts || ts->ndata == 0 || !ts->datas) return NULL;
    if(ts->type != TENSOR_TYPE_FLOAT32 && ts->type != TENSOR_TYPE_INT8) return NULL;
    float* val_vec = vector_create();
    float* cnt_vec = vector_create();
    ...
    if(ts->type == TENSOR_TYPE_FLOAT32) {
        ...
        part = (max - min) / len;
        for(int j = 0; j < len; j++) {
            vector_add(&val_vec, min + j * part);
            vector_add(&cnt_vec, 0.f);
            for(int i = 0; i < ts->ndata; i++) {
                if(data[i] >= min + j * part && data[i] < min + (j + 1) * part) {
                    cnt_vec[j] = cnt_vec[j] + 1.f;
                }
                if(i == ts->ndata - 1 && data[i] == min + (j + 1) * part) {
                    cnt_vec[j] = cnt_vec[j] + 1.f;
                }
            }
        }
    } else if(ts->type == TENSOR_TYPE_INT8) {
        int8_t* data = ts->datas;
        min = (float)data[0];
        max = (float)data[0];
        for(int i = 0; i < ts->ndata; i++) {
        ...
        for(int j = 0; j < len; j++) {
            ...
            vector_add(&cnt_vec, 0.f);
            for(int i = 0; i < ts->ndata; i++) {
                if((float)data[i] >= min + j * part && (float)data[i] < min + (j + 1) * part) {
                    cnt_vec[j] = cnt_vec[j] + 1.f;
                }
                if(i == ts->ndata - 1 && (float)data[i] == min + (j + 1) * part) {
                    cnt_vec[j] = cnt_vec[j] + 1.f;
                }
            }
        }
    ...
    char name_buf[64];
    len = vector_size(val_vec);

    sprintf(name_buf, "%s_hist", ts->name);
    tensor_t* new_ts = tensor_new(name_buf, TENSOR_TYPE_FLOAT32);
    tensor_reshape(new_ts, 2, (int[]){len, 2});
    float* hist = malloc(2 * len * sizeof(float));
    for(int i = 0; i < len; i++) {
        hist[i * 2]     = val_vec[i];
        hist[i * 2 + 1] = cnt_vec[i];
    }
    tensor_apply(new_ts, hist, 2 * len * sizeof(float));
    free(hist);
...
tensor_t* tensor_quant(tensor_t* ts, tensor_t* scale_ts, tensor_t* zero_point_ts, int axis, tensor_type_t otype) {
    if(!ts || !scale_ts) return ts;
    node_t* nd = node_temp("quant", OP_TYPE_QUANTIZE_LINEAR);
    nd->nin = zero_point_ts ? 3 : 2;
    nd->nout= 1;
    nd->in = sys_malloc(nd->nin * sizeof(tensor_t*));
    nd->out = sys_malloc(nd->nout * sizeof(tensor_t*));
    nd->in[0] = ts;
    ...
    char name_buf[64];
    name_buf[0] = '\0';
    sprintf(name_buf, "%s_q", ts->name);
    nd->out[0] = tensor_new(name_buf, TENSOR_TYPE_UNDEFINED);
    if(zero_point_ts) {
        nd->in[2] = zero_point_ts;
    }
    attribute_t* axis_attr = attribute_int("axis", axis);
    attribute_t* output_dtype_attr = attribute_int("output_dtype", (int64_t)otype);
    vector_add(&nd->attr_vec, axis_attr);
    vector_add(&nd->attr_vec, output_dtype_attr);
    node_bind_op(nd);
    if(nd->op && nd->op->init) {  
        nd->op->init(nd);
        nd->op->reshape(nd);
...
tensor_t* tensor_dequant(tensor_t* ts, tensor_t* scale_ts, tensor_t* zero_point_ts, int axis) {
    if(!ts || !scale_ts) return ts;
    node_t* nd = node_temp("dequant", OP_TYPE_DEQUANTIZE_LINEAR);
    nd->nin = zero_point_ts ? 3 : 2;
    nd->nout= 1;
    nd->in = sys_malloc(nd->nin * sizeof(tensor_t*));
    nd->out = sys_malloc(nd->nout * sizeof(tensor_t*));
    nd->in[0] = ts;
    nd->in[1] = scale_ts;
    nd->out[0] = tensor_new("dequant_out", TENSOR_TYPE_UNDEFINED);
    if(zero_point_ts) {
        nd->in[2] = zero_point_ts;
    }
    attribute_t* axis_attr = attribute_int("axis", axis);
...
int tensor_reshape(tensor_t *ts, int ndim, int *dims) {
    ...
    ts->ndim = ndim;
    ts->ndata = new_ndata;
    if (old_ndata != new_ndata || !ts->datas) {
...
void tensor_boardcast(tensor_t* x,tensor_t* y) {
    ...
    int cp_len = indim[0];
    int tbyte = tensor_type_sizeof(x->type);
    int cp_byte = cp_len*tbyte;
    ...
    for(int i=0; i<indim[gs-1]/indim[0]; i++){
        ...
        memcpy(y->datas + out_off*tbyte, x->datas + in_off*tbyte, cp_byte);
        offs[i] = out_off;
    }
    for(int i=0; i<gs; i++){
        for(int j=0; j<indim[gs-1]/indim[0]; j++){
            if(offs[j] % outdim[i] == 0){   
                ...
                cp_byte = cp_len*tbyte;
                int from = offs[j];
                int at = from + cp_len;
                int end = from + outdim[i];
                while(at + cp_len <= end){
                    memcpy(y->datas+at*tbyte, y->datas+from*tbyte, cp_byte);
                    at += cp_len;
                    cp_len <<= 1;
                    cp_byte <<= 1;
                }
...
void tensor_save(tensor_t* ts, const char* path, const char* mode) {
    if(!ts || !ts->datas || !path) return;
    char* ext = sys_get_file_ext(path);
    if(strcmp(ext, "c") == 0 || strcmp(ext, "w") == 0) {
        FILE* fp = fopen(path, mode);
        char* shape = tensor_dump_shape(ts);
        char name_buf[64];
        int name_len = 0;
        for(int i = 0; i < strlen(ts->name); i++) {
            char c = ts->name[i];
            if((c <= 'Z' && c >= 'A')
                || (c <= 'z' && c >= 'a')
                || (c <= '9' && c >= '0')
                || (c == '_')) {
                name_buf[name_len] = ts->name[i];
                name_len++;
            }
        }
        name_buf[name_len] = '\0';
        fprintf(fp, "// %s %s\n", ts->name, shape);
        free(shape);
        // fprintf(fp, "static unsigned int %s_len = %lu;\n" , name_buf, ts->ndata);
        switch(ts->type) {
            case TENSOR_TYPE_FLOAT32: {
                fprintf(fp, "static float %s_val[%lu] = {\n    ", name_buf, ts->ndata);
                for(int i = 0; i < ts->ndata; i++) {
                    fprintf(fp, "%f,", ((float*)(ts->datas))[i]);
                    for(int j = 0; j < ts->ndim - 1; j++) {
                        if((i + 1) % ts->strides[j] == 0) fprintf(fp, "\n    ");
                    }
                }
                break;
            }
            case TENSOR_TYPE_INT32: {
                fprintf(fp, "static int %s_val[%lu] = {\n    ", name_buf, ts->ndata);
                for(int i = 0; i < ts->ndata; i++) {
                    fprintf(fp, "%d,", ((int32_t*)(ts->datas))[i]);
                    for(int j = 0; j < ts->ndim - 1; j++) {
                        if((i + 1) % ts->strides[j] == 0) fprintf(fp, "\n    ");
                    }
                }
            ...
            case TENSOR_TYPE_INT16: {
                fprintf(fp, "static short %s_val[%lu] = {\n    ", name_buf, ts->ndata);
                for(int i = 0; i < ts->ndata; i++) {
                    fprintf(fp, "%d,", ((int16_t*)(ts->datas))[i]);
                    for(int j = 0; j < ts->ndim - 1; j++) {
                        if((i + 1) % ts->strides[j] == 0) fprintf(fp, "\n    ");
                    }
                }
            ...
            case TENSOR_TYPE_INT8: {
                fprintf(fp, "static char %s_val[%lu] = {\n    ", name_buf, ts->ndata);
                for(int i = 0; i < ts->ndata; i++) {
                    fprintf(fp, "%d,", ((int8_t*)(ts->datas))[i]);
                    for(int j = 0; j < ts->ndim - 1; j++) {
                        if((i + 1) % ts->strides[j] == 0) fprintf(fp, "\n    ");
                    }
                }
...
void tensor_dump3(tensor_t *ts) {
    if (!ts) return;
    void *p;
    int i;
    if (ts->name) {
        LOG_INFO("{\n\"name\": \"%s\",\n\"dtype\": \"%s\",\n\"layout\": \"%s\",\n", strcmp(ts->name, "") == 0 ? "Tensor" : ts->name, tensor_type_tostring(ts->type), tensor_layout_tostring(ts->layout));
        LOG_INFO("\"ndim\": %d,\n\"dims\": [", ts->ndim);
        for(i = 0; i < ts->ndim; i++) {
            LOG_INFO("%d", ts->dims[i]);
            if(i != ts->ndim - 1) {
                LOG_INFO(",");
            }
        }
        if(ts->is_param && ts->datas) {
            LOG_INFO("],\n\"ndata\": %lu,\n\"datas\": [", ts->ndata);
            for (i = 0; i < ts->ndim; i++) {
                if (ts->dims[i] <= 0)
                    return;
