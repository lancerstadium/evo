...
static int rv_decode4(uint32_t inst, RvInst* restrict rv_inst) {
  enum {
    ENC_F_RD  = 1 << 0,
    ENC_F_RS1 = 1 << 1,
    ENC_F_RS2 = 1 << 2,
  ...
  case 0x00: encoding = ENC_I;
    mnem = (const uint16_t[]) {RV_LB, RV_LH, RV_LW, RV_LD, RV_LBU, RV_LHU, RV_LWU, 0}[funct3];
    break;
  case 0x01: encoding = ENC_I;
    mnem = (const uint16_t[]) {0, 0, RV_FLW, RV_FLD, 0, 0, 0, 0}[funct3];
    break;
  case 0x03: encoding = ENC_I;
    mnem = (const uint16_t[]) {RV_FENCE, RV_FENCEI, 0, 0, 0, 0, 0, 0}[funct3];
    break;
  case 0x04:
    switch (funct3) {
  ...
  case 0x08: encoding = ENC_S;
    mnem = (const uint16_t[]) {RV_SB, RV_SH, RV_SW, RV_SD, 0, 0, 0, 0}[funct3];
    break;
  case 0x09: encoding = ENC_S;
    mnem = (const uint16_t[]) {0, 0, RV_FSW, RV_FSD, 0, 0, 0, 0}[funct3];
    break;
  case 0x0b: encoding = ENC_R | ENC_F_IMM_AMO;
    switch (funct7 >> 2) {
    case 0x00: mnem = (const uint16_t[]) {0, 0, RV_AMOADDW, RV_AMOADDD, 0, 0, 0, 0}[funct3]; break;
    case 0x01: mnem = (const uint16_t[]) {0, 0, RV_AMOSWAPW, RV_AMOSWAPD, 0, 0, 0, 0}[funct3]; break;
    case 0x02: mnem = (const uint16_t[]) {0, 0, RV_LRW, RV_LRD, 0, 0, 0, 0}[funct3]; break;
    case 0x03: mnem = (const uint16_t[]) {0, 0, RV_SCW, RV_SCD, 0, 0, 0, 0}[funct3]; break;
    case 0x04: mnem = (const uint16_t[]) {0, 0, RV_AMOXORW, RV_AMOXORD, 0, 0, 0, 0}[funct3]; break;
    case 0x08: mnem = (const uint16_t[]) {0, 0, RV_AMOORW, RV_AMOORD, 0, 0, 0, 0}[funct3]; break;
    case 0x0c: mnem = (const uint16_t[]) {0, 0, RV_AMOANDW, RV_AMOANDD, 0, 0, 0, 0}[funct3]; break;
    case 0x10: mnem = (const uint16_t[]) {0, 0, RV_AMOMINW, RV_AMOMIND, 0, 0, 0, 0}[funct3]; break;
    case 0x14: mnem = (const uint16_t[]) {0, 0, RV_AMOMAXW, RV_AMOMAXD, 0, 0, 0, 0}[funct3]; break;
    case 0x18: mnem = (const uint16_t[]) {0, 0, RV_AMOMINUW, RV_AMOMINUD, 0, 0, 0, 0}[funct3]; break;
    case 0x1c: mnem = (const uint16_t[]) {0, 0, RV_AMOMAXUW, RV_AMOMAXUD, 0, 0, 0, 0}[funct3]; break;
    default: return RV_UNDEF;
    }
    break;
  case 0x0c: encoding = ENC_R;
    switch (funct7) {
    case 0x00: mnem = (const uint16_t[]) {RV_ADD, RV_SLL, RV_SLT, RV_SLTU, RV_XOR, RV_SRL, RV_OR, RV_AND}[funct3]; break;
    case 0x01: mnem = (const uint16_t[]) {RV_MUL, RV_MULH, RV_MULHSU, RV_MULHU, RV_DIV, RV_DIVU, RV_REM, RV_REMU}[funct3]; break;
    case 0x20: mnem = (const uint16_t[]) {RV_SUB, 0, 0, 0, 0, RV_SRA, 0, 0}[funct3]; break;
    default: return RV_UNDEF;
    }
    break;
  ...
  case 0x0e: encoding = ENC_R;
    switch (funct7) {
    case 0x00: mnem = (const uint16_t[]) {RV_ADDW, RV_SLLW, 0, 0, 0, RV_SRLW, 0, 0}[funct3]; break;
    case 0x01: mnem = (const uint16_t[]) {RV_MULW, 0, 0, 0, RV_DIVW, RV_DIVUW, RV_REMW, RV_REMUW}[funct3]; break;
    case 0x20: mnem = (const uint16_t[]) {RV_SUBW, 0, 0, 0, 0, RV_SRAW, 0, 0}[funct3]; break;
    default: return RV_UNDEF;
    }
    break;
  case 0x10: encoding = ENC_R4 | ENC_F_RM; mnem = (const uint16_t[4]) {RV_FMADDS, RV_FMADDD}[UBFX(inst, 25, 26)]; break;
  case 0x11: encoding = ENC_R4 | ENC_F_RM; mnem = (const uint16_t[4]) {RV_FMSUBS, RV_FMSUBD}[UBFX(inst, 25, 26)]; break;
  case 0x12: encoding = ENC_R4 | ENC_F_RM; mnem = (const uint16_t[4]) {RV_FNMSUBS, RV_FNMSUBD}[UBFX(inst, 25, 26)]; break;
  case 0x13: encoding = ENC_R4 | ENC_F_RM; mnem = (const uint16_t[4]) {RV_FNMADDS, RV_FNMADDD}[UBFX(inst, 25, 26)]; break;
  case 0x14:
    switch (funct7) {
    ...
    case 0x20: encoding = ENC_R2 | ENC_F_RM; mnem = (const uint16_t[32]) {0, RV_FCVTSD}[UBFX(inst, 20, 24)]; break;
    case 0x21: encoding = ENC_R2 | ENC_F_RM; mnem = (const uint16_t[32]) {RV_FCVTDS}[UBFX(inst, 20, 24)]; break;
    ...
    case 0x10: encoding = ENC_R; mnem = (const uint16_t[]) {RV_FSGNJS, RV_FSGNJNS, RV_FSGNJXS, 0, 0, 0, 0, 0}[funct3]; break;
    case 0x11: encoding = ENC_R; mnem = (const uint16_t[]) {RV_FSGNJD, RV_FSGNJND, RV_FSGNJXD, 0, 0, 0, 0, 0}[funct3]; break;
    case 0x14: encoding = ENC_R; mnem = (const uint16_t[]) {RV_FMINS, RV_FMAXS, 0, 0, 0, 0, 0, 0}[funct3]; break;
    case 0x15: encoding = ENC_R; mnem = (const uint16_t[]) {RV_FMIND, RV_FMAXD, 0, 0, 0, 0, 0, 0}[funct3]; break;
    case 0x50: encoding = ENC_R; mnem = (const uint16_t[]) {RV_FLES, RV_FLTS, RV_FEQS, 0, 0, 0, 0, 0}[funct3]; break;
    case 0x51: encoding = ENC_R; mnem = (const uint16_t[]) {RV_FLED, RV_FLTD, RV_FEQD, 0, 0, 0, 0, 0}[funct3]; break;
    case 0x60: encoding = ENC_R2 | ENC_F_RM; mnem = (const uint16_t[32]) {RV_FCVTWS, RV_FCVTWUS, RV_FCVTLS, RV_FCVTLUS}[UBFX(inst, 20, 24)]; break;
    case 0x61: encoding = ENC_R2 | ENC_F_RM; mnem = (const uint16_t[32]) {RV_FCVTWD, RV_FCVTWUD, RV_FCVTLD, RV_FCVTLUD}[UBFX(inst, 20, 24)]; break;
    case 0x68: encoding = ENC_R2 | ENC_F_RM; mnem = (const uint16_t[32]) {RV_FCVTSW, RV_FCVTSWU, RV_FCVTSL, RV_FCVTSLU}[UBFX(inst, 20, 24)]; break;
    case 0x69: encoding = ENC_R2 | ENC_F_RM; mnem = (const uint16_t[32]) {RV_FCVTDW, RV_FCVTDWU, RV_FCVTDL, RV_FCVTDLU}[UBFX(inst, 20, 24)]; break;
    case 0x70: encoding = ENC_R2; mnem = (const uint16_t[]) {RV_FMVXW, RV_FCLASSS, 0, 0, 0, 0, 0, 0}[funct3]; break; // TODO: check rs2
    case 0x71: encoding = ENC_R2; mnem = (const uint16_t[]) {RV_FMVXD, RV_FCLASSD, 0, 0, 0, 0, 0, 0}[funct3]; break; // TODO: check rs2
    case 0x78: encoding = ENC_R2; mnem = (const uint16_t[]) {RV_FMVWX, 0, 0, 0, 0, 0, 0, 0}[funct3]; break; // TODO: check rs2
    case 0x79: encoding = ENC_R2; mnem = (const uint16_t[]) {RV_FMVDX, 0, 0, 0, 0, 0, 0, 0}[funct3]; break; // TODO: check rs2
    default: return RV_UNDEF;
    }
    break;
  case 0x18: encoding = ENC_B;
    mnem = (const uint16_t[]) {RV_BEQ, RV_BNE, 0, 0, RV_BLT, RV_BGE, RV_BLTU, RV_BGEU}[funct3];
    break;
  ...
  case 0x1c: encoding = ENC_I;
    mnem = (const uint16_t[]) {RV_ECALL, RV_CSRRW, RV_CSRRS, RV_CSRRC, 0, RV_CSRRWI, RV_CSRRSI, RV_CSRRCI}[funct3];
    break;
  }
...
static int rv_decode2(uint16_t inst, RvOptions opt,
                       RvInst* restrict rv_inst) {
  enum {
    ENC_I_NONE = 0,
    ENC_U_53_76,
    ENC_U_53_26,
  ...
  case 0x8001:
    ...
    case 3:
      ...
      mnem = (const uint16_t[8]) {RV_SUB, RV_XOR, RV_OR, RV_AND, RV_SUBW, RV_ADDW}[mnem];
      break;
    }
    break;
  case 0xa001:
  ...
  if (imm_enc >= ENC_NZ_START && !rv_inst->imm) {
    ...
    if (rv_inst->rd != 0 || rv_inst->mnem != RV_ADDI) {
      return RV_UNDEF;
    }
  }
  return 2;
...
int rv_decode(size_t bufsz, const uint8_t* buf, RvOptions opt,
               RvInst* restrict rv_inst) {
  if (UNLIKELY(bufsz < 2))
    return RV_PARTIAL;
  if ((buf[0] & 0x03) != 0x03)
...
void rv_format(const RvInst* inst, size_t len, char* restrict buf) {
  char tmp[18];
  if (!len)
    return;
  buf[0] = 0;
