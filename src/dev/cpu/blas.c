        ...
        for(int c=0; c<C; c++){
            for(int oh=0; oh<out_H; oh++){
                for(int ow=0; ow<out_W; ow++){
                    float in_x = (ow + 0.5f) * ((float)in_W / (float)out_W) - 0.5f;
                    float in_y = (oh + 0.5f) * ((float)in_H / (float)out_H) - 0.5f;
                    int ix = (int)in_x;
                    int iy = (int)in_y;
                    ix = ix < 0 ? 0 : (ix < in_W ? ix : in_W);
                    iy = iy < 0 ? 0 : (iy < in_H ? iy : in_H);
                    int ix1 = ix < in_W-1 ? in_x + 1 : in_x;
                    int iy1 = iy < in_H-1 ? in_y + 1 : in_y;
                    float dx = in_x - ix;
                    float dy = in_y - iy;
                    float xy0 = X[((n*C+c)*in_W+iy)*in_W+ix];
                    float xy1 = X[((n*C+c)*in_W+iy)*in_W+ix1];
                    float xy2 = X[((n*C+c)*in_W+iy1)*in_W+ix];
                    float xy3 = X[((n*C+c)*in_W+iy1)*in_W+ix1];
                    float line1 = xy0 + dx * (xy1 - xy0);
                    float line2 = xy2 + dx * (xy3 - xy2);
                    Y[((n*C+c)*out_H+oh)*out_W+ow] = line1 + dy * (line2 - line1);
                }
            }
        }
    }
