...
void Resize_reshape(node_t* nd) {
    ...
    pdat->coordinate_transformation_mode = node_get_attr_string(nd, "coordinate_transformation_mode", "half_pixel");
    pdat->cubic_coeff_a = node_get_attr_float(nd, "cubic_coeff_a", -0.75);
    pdat->mode = node_get_attr_string(nd, "mode", "nearest");
    pdat->nearest_mode = node_get_attr_string(nd, "nearest_mode", "round_prefer_floor");
    switch(shash(pdat->mode)){
        ...
        case 0x0b7641e0: {
            ...
            int ndim = sc->ndata;
            int new_dims[ndim];
            int64_t* sc_datas = sc->datas;
            for (int i = 0; i < ndim; i++) {
                new_dims[i] = sc_datas[i];
            }
            tensor_reshape(y, ndim, new_dims);
        }
            break; /* linear */
            /* cubic */
        default: break;
...
void Resize_forward_float32(node_t* nd) {
    ...
    switch(shash(pdat->mode)) {
        case 0x09fa48d7: {
            ...
            float *sc = nd->in[2]->datas;
            tensor_t *y = nd->out[0];
            Resize_nearest_float32_cpu(x->datas, y->datas, x->dims[0], x->dims[1], x->dims[2], x->dims[3], 1, sc[3], true);
        } break; /* nearest */
        case 0x0b7641e0: {
            tensor_t *x = nd->in[0];
            tensor_t *y = nd->out[0];
            Resize_linear_float32_cpu(x->datas, y->datas, y->dims[0], y->dims[1], x->dims[2], x->dims[3], y->dims[2], y->dims[3]);
        } break; /* linear */
        /* cubic */
        default: break;
    }
