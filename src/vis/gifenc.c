...
static void ge_apply_alpha(uint8_t *r, uint8_t *g, uint8_t *b, uint8_t a) {
...
static uint8_t ge_quantify_color256(uint32_t c) {
    // Extract the RGBA components from the input color in (a, b, g, r) format
    uint8_t a = (c >> 24) & 0xFF;
    uint8_t b = (c >> 16) & 0xFF;
    uint8_t g = (c >> 8) & 0xFF;
    uint8_t r = c & 0xFF;

    // Apply the alpha channel to the RGB components
    ge_apply_alpha(&r, &g, &b, a);

    // Convert the RGB components to 6x6x6 cube (216 color "web-safe" palette)
    uint8_t r6 = (r * 5) / 255;
    uint8_t g6 = (g * 5) / 255;
    uint8_t b6 = (b * 5) / 255;

    // Calculate the index for the 216 color "web-safe" part of the palette
    uint8_t web_safe_index = 16 + (r6 * 36) + (g6 * 6) + b6;

    // Check if the color is in the grayscale range
    if (r == g && g == b) {
        // Calculate the index for the grayscale part of the palette
        uint8_t grayscale_index = (r * 23) / 255;
        if (grayscale_index < 24) {
            return 232 + grayscale_index;
        }
    }

    // Return the index in the 256 color palette
    return web_safe_index;
}
...
static uint8_t ge_match_color(ge_GIF *gif, uint8_t *color, int channel) {
    ...
    if(gif->palette == NULL) {
        return ge_quantify_color256(loc_color);
    } else {
        for(int i = 0; i < all; i++) {
