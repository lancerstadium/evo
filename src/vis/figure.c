...
static void svg_plot2d_bar(char* buffer, figure_t* fig, figure_plot_t* p, size_t idx) {
    ...
    float y0 = svg_plot2d_y(0, fig->priv, fig->axiss[1]);
    float w = p->bar.bwidth;
    float* fs = p->data->datas;
    for(int i = 0; i < n; i++) {
        float x = fs[i * na];
        float y = fs[i * na + 1];
        svg_bar(buffer, p->lwidth, color, p->bar.lcolor, svg_plot2d_x(x, fig->priv, fig->axiss[0]), svg_plot2d_y(y, fig->priv, fig->axiss[1]), y0, w, "plot-area");
    }
    if(!p->color) free(color);
}
    ...
    for(int i = 0; i < vector_size(fig->plot_vec); i++) {
        ...
        if(p->label) {
            ...
            if(p->type == FIGURE_PLOT_TYPE_BAR) {
                ...
                float xb = x+10+w/2;
                float yb = y - 8;
                svg_bar(buffer, p->lwidth, color, p->bar.lcolor, xb, yb, y0, w, "plot-area");
            }
            if(!p->color) {
                free(color);
            }
...
figure_plot_t* figure_plot_new(char* label, figure_plot_type_t type, float* fs, size_t nplot, size_t naxis) {
    ...
    fig_plot->data = data;
    fig_plot->lwidth = 2.0f;
    fig_plot->ltype = FIGURE_LINE_TYPE_NORMAL;
    if(type == FIGURE_PLOT_TYPE_SCATTER) {
        fig_plot->mtype = 'o';
    } else if(type == FIGURE_PLOT_TYPE_BAR) {
        fig_plot->bar.lcolor = sys_strdup("#000000");
        fig_plot->bar.bwidth = 35;
    }
    return fig_plot;
}
...
void figure_plot_free(figure_plot_t* fig_plot) {
    if(fig_plot) {
        ...
        if(fig_plot->type == FIGURE_PLOT_TYPE_BAR) {
            if(fig_plot->bar.lcolor) { free(fig_plot->bar.lcolor); fig_plot->bar.lcolor = NULL; }
        }
        fig_plot->priv = NULL;
        free(fig_plot);
        fig_plot = NULL;
...
figure_t* figure_new_1d(char* title, figure_type_t type, figure_plot_type_t ptype, tensor_t* ts) {
    ...
    figure_add_plot_1d(fig, ptype, ts, -1);
    return fig;
}

figure_t* figure_new_2d(char* title, figure_type_t type, figure_plot_type_t ptype, tensor_t* ts) {
    if(!ts || ts->ndim < 1 || ts->type != TENSOR_TYPE_FLOAT32) return NULL;
    figure_t* fig = figure_new(title, type, 720, 480, 2);
    figure_set_xlabel(fig, "x");
    figure_set_ylabel(fig, ts->name);
    figure_add_plot_2d(fig, ptype, ts, -1);
    return fig;
}

void figure_add_plot_1d(figure_t* fig, figure_plot_type_t ptype, tensor_t* ts, int max) {
    ...
    figure_plot_t* p = figure_plot_new(ts->name, ptype, data, n, 2);
    figure_add_plot(fig, p);
}

void figure_add_plot_2d(figure_t* fig, figure_plot_type_t ptype, tensor_t* ts, int max) {
    if(!fig || !ts || ts->ndim < 1 || ts->type != TENSOR_TYPE_FLOAT32) return;
    int n = ts->dims[0];
    int s = ts->strides[0];
    if(max > 0 && max < n) {
    ...
    for(int i = 0; i < n; i++) {
        data[i * 2]     = datas[i * s];
        data[i * 2 + 1] = datas[i * s + 1];
    }
    figure_plot_t* p = figure_plot_new(ts->name, ptype, data, n, 2);
    figure_add_plot(fig, p);
}

void figure_update_plot_1d(figure_t* fig, tensor_t* ts, int max) {
    if(!fig || !ts) return;
    figure_plot_type_t ptype = fig->plot_vec[vector_size(fig->plot_vec) - 1]->type;
    figure_pop_plot(fig);
    figure_add_plot_1d(fig, ptype, ts, max);
}
