...
image_t* image_from_tensor(tensor_t *ts) {
    ...
    if(img && ts) {
        tensor_t *new_ts = tensor_new(ts->name, TENSOR_TYPE_UINT8);
        int ndim = 4;
        int dims[4];
        dims[0] = 1;
        if(ts->ndim <= 2) {
        ...
        uint8_t in_layout = ts->layout;
        tensor_t * tss;
        if(in_layout != 1) {
            tss = tensor_nchw2nhwc(ts);
        } else {
            tss = ts;
        }
        if(tss->ndim == 3) {
        ...
        new_ts->layout = 1;
        if(tss->type == TENSOR_TYPE_FLOAT32) {
            float* data = tss->datas;
            uint8_t* datas = new_ts->datas;
            for(int i = 0; i < new_ts->ndata; i++) {
                datas[i] = (uint8_t)data[i];
            }
        } else if(tss->type == TENSOR_TYPE_UINT8) {
            uint8_t* data = tss->datas;
            uint8_t* datas = new_ts->datas;
            for(int i = 0; i < new_ts->ndata; i++) {
                datas[i] = (uint8_t)data[i];
            }
        }
        img->name = sys_strdup(tss->name);
...
image_t* image_blank(const char* name, size_t width, size_t height) {
    ...
    if(img) {
        ...
        img->attr_vec = vector_create();
        img->type = IMAGE_TYPE_UNKNOWN;
        img->raw = tensor_new(name, TENSOR_TYPE_UINT8);
...
image_t* image_extract_channel(image_t* img, int channel) {
    ...
    if(new_img && img && img->raw->layout == 1) {
        ...
        new_img->attr_vec = vector_create();
        new_img->type = img->type;
        new_img->raw = tensor_new(img->name, TENSOR_TYPE_UINT8);
    ...
    if(img && name && img->raw) {
        ...
        switch(image_get_type(name)) {
            ...
            case IMAGE_TYPE_TIFF:
                tiff_encode(new_ts->datas, name, new_ts->dims[2], new_ts->dims[1], new_ts->dims[3], 8);
                LOG_INFO("Image save: %s\n", name);
                break;
            case IMAGE_TYPE_GIF:
...
void image_crop(image_t* img, int start_x, int start_y, int crop_width, int crop_height) {
    ...
    // 限制裁剪区域在图像范围内
    crop_height = (crop_height <= height - start_y) ? crop_height : height - start_y;
    crop_width = (crop_width <= width - start_x) ? crop_width : width - start_x;
    if (start_x < 0 || start_y < 0 || start_x >= width || start_y >= height) return;

    // 分配新数据空间用于存储裁剪结果
    unsigned char* data = (unsigned char*)sys_malloc(crop_height * crop_width * channel);

    // 根据起始点和裁剪宽高进行裁剪
    uint8_t* datas = img->raw->datas;
    for (int i = 0; i < crop_height; i++) {
        int src_index = ((start_y + i) * width + start_x) * channel;
        memcpy(data + i * crop_width * channel, datas + src_index, crop_width * channel);
    }
