...
static op_type_t op_map_onnx(char *op_ty) {
    ...
    switch (shash(op_ty)) {
        ...
        // ============ acc:start =========== //
        case 0x508b8be8:
            return OP_TYPE_TRI;    /* "Tri" */
        // ============ acc:end =========== //
        // ============ qnn:start =========== //
        case 0xd77a2d37:
            return OP_TYPE_QCONV; /* "QConv" */
        case 0x0dfa948e:
            return OP_TYPE_QGEMM; /* "QGemm" */
        // ============ qnn:end   =========== //
        default:
            return OP_TYPE_NOP;
    }
...
model_t *load_onnx(struct serializer *s, const void *buf, size_t len) {
    ...
    mdl->vmodel = onnx__model_proto__unpack(NULL, len, buf);
    if (!mdl->vmodel) {
        if (mdl)
            sys_free(mdl);
        return NULL;
    ...
    mdl->model_size = len;
    mdl->name = sys_strdup(((Onnx__ModelProto*)(mdl->vmodel))->domain);

    mdl->tensor_map = hashmap_create();
    if (!mdl->tensor_map) {
        if (mdl->vmodel) {
            onnx__model_proto__free_unpacked(mdl->vmodel, NULL);
        }
        if (mdl)
            sys_free(mdl);
        return NULL;
...
static tensor_t *tensor_from_value_info(Onnx__ValueInfoProto *v) {
    tensor_t *t;
    tensor_type_t type;
    int *dims = NULL;
    int ndim;
...
graph_t *load_graph_onnx(model_t *mdl) {
    ...
    Onnx__GraphProto *graph = ((Onnx__ModelProto *)(mdl->vmodel))->graph;
    graph_t *g;
    node_t *n;
    tensor_t *t;
    Onnx__TensorProto *o;
    ...
    for (i = 0; i < g->nnode; i++) {
        ...
        if (!domain || (strlen(domain) == 0))
            domain = "ai.onnx";
        for (j = 0; j < ((Onnx__ModelProto *)(mdl->vmodel))->n_opset_import; j++) {
            p = ((Onnx__ModelProto *)(mdl->vmodel))->opset_import[j]->domain;
            if (!p || (strlen(p) == 0))
                p = "ai.onnx";
            if (strcmp(domain, p) == 0) {
                n->opset = ((Onnx__ModelProto *)(mdl->vmodel))->opset_import[j]->version;
                break;
            }
        }
