...
static struct bucket* resize_entry(hashmap_t* m, struct bucket* old_entry) {
...
static int hashmap_resize(hashmap_t* m) {
...
static struct bucket* find_entry(hashmap_t* m, const void* key, size_t ksize, uint32_t hash) {
...
int hashmap_set(hashmap_t* m, const void* key, size_t ksize, uintptr_t val) {
...
int hashmap_get_set(hashmap_t* m, const void* key, size_t ksize, uintptr_t* out_in) {
    if (m->count + 1 > HASHMAP_MAX_LOAD * m->capacity) {
        if (hashmap_resize(m) == -1)
            return -1;
    }
...
int hashmap_set_free(hashmap_t* m, const void* key, size_t ksize, uintptr_t val, hashmap_callback c, void* usr) {
    if (m->count + 1 > HASHMAP_MAX_LOAD * m->capacity) {
        if (hashmap_resize(m) == -1)
            return -1;
    }
...
int hashmap_get(hashmap_t* m, const void* key, size_t ksize, uintptr_t* out_val) {
    uint32_t hash = hash_data(key, ksize);
    struct bucket* entry = find_entry(m, key, ksize, hash);
...
void hashmap_remove(hashmap_t* m, const void* key, size_t ksize) {
    uint32_t hash = hash_data(key, ksize);
    struct bucket* entry = find_entry(m, key, ksize, hash);
...
int hashmap_iterate(hashmap_t* m, hashmap_callback c, void* user_ptr) {
