...
ifeq ($(CROSS_COMPILE),riscv64-linux-gnu-)
	...
	ARCH_DEP := 
	LIBDIRS += -L/usr/aarch64-linux-gnu/lib/aarch64-linux-gnu/
	INCDIRS += -I/usr/aarch64-linux-gnu/include
endif
...
ifneq ($(CODEGEN_ENB),)
	@echo "#define EVO_CODEGEN_ENB" >> $(CFGFILE)
	@echo "  - codegen:  \t\t\e[32;1mon\e[0m"
else
	@echo "  - codegen:  \t\t\e[31;1moff\e[0m"
endif
ifneq ($(ACC_ENB_RES),)
ifeq ($(ACC_ENB_CUDA),1)
...
release:
	...
	@cp $(TRGDIR)/libevo.a $(TRGDIR)/evo-release-$(VERSION)/
